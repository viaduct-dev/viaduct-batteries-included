...
@Resolver
class PreviewAssetSyncResolver(
...
) : MutationResolvers.PreviewAssetSync() {
    override suspend fun resolve(ctx: Context): SyncJob {
        ...
        if (repoPair != null) {
            ...
            val job = viaAccessService.createSyncJob(ctx.authenticatedClient, assetId, asset.asset_type, asset.tenant_name, "PREVIEW_ASSET")
            if (githubSyncExecutor != null) {
                CoroutineScope(Dispatchers.IO).launch {
                    githubSyncExecutor.executeRepoSync(ctx.authenticatedClient, job, repoAsset.owner, repoAsset.repo, preview = true)
                }
            }
